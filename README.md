...
**APIキー不要・クレジットカード不要・無料**。Google Maps は課金事故の危険があるため見送りました。住所は約100m動いたときだけ引き直し、取れなければ**空のままにします**。それらしい住所をでっち上げると、地図の位置と住所が食い違って原因不明の不具合になります。

⚠️ 当初はタイルに CARTO を使っていましたが、キー無しのタイルに「API KEY REQUIRED」の透かしが入る方針に変わり、**デプロイして初めて地図が読めなくなっているのに気づきました**。外部サービスの「無料」は、こちらの都合で変わらない保証がない——選定理由に無料を置くなら、変わったときの移行先まで決めておくべきでした。国土地理院に暗色版は無いので、夜テーマは同じタイルを CSS フィルタで反転しています。
